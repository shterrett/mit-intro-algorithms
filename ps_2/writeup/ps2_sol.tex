...
%%% Assumption of inifinite resolution implies
%%% infinite number of points on any line segment.
%%% This algorithm is thus Θ(∞), and all following
...
Because the triangles are compiled first and only rasterized at the end
of the process, the rendering is a constant-time operation.

The cost of rasterizing each triangle is proportional to its area, but
the total area of the triangles is bounded by the area of the original triangle
that is rendered in step $-1$. Therefore, the total cost is bounded by a constant
and because it is assumed that this cost is greater than compiling the
list of triangles, it dominates and gives Θ(1).

My confusion in this proof and the problems before was that the actual
drawing did not happen on each step, but only the last. Therefore, I thought
that the total area being drawn would increase with each recursion of the problem
because triangles were begin subdivided and re-drawn.
