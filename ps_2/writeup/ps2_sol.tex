...
Rasterizing a triangle is proportional to the area $A$ of a triangle.
$A = \frac{\sqrt{3}}{4} \times l^2$ where $l$ is the side length.
The $ith$ recursive step creates one triangle of side length
$\frac{l}{3^i}$ and 4 problems in the $i+1 th$ step.
Therefore, the total work done at any step $i$ is
$4^i \times \frac{\sqrt{3}}{4} \times \frac{l}{3^i}$

This grows as $4^i$, which gives a total complexity of Θ($4^n$).
