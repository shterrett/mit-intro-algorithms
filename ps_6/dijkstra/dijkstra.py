...
class Network(object):
    ...
    def verify_path(self, path, source, destination):
        ...
        """
        if source != path[0]:
            raise ValueError('First node on path is different from the source.')
        if destination != path[-1]:
            raise ValueError('Last node on path is different from the destination.')
...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        """
        def path_length(predecessor, successor):
          return predecessor.key.distance + weight(predecessor, successor)
        ...
        for node in nodes:
          node.key = NodeDistancePair(node, INFINITY)
          node.predecessor = None
          queue.insert(node.key)

        source.key.distance = 0
        queue.decrease_key(source.key)
        current_node = source.key

        while len(queue) > 0 and current_node.node != destination:
          current_node = queue.extract_min()
          visited.add(current_node)
          for neighbor in current_node.node.adj:
            alternative_distance = path_length(current_node.node, neighbor)
            if neighbor.key.distance > alternative_distance:
              neighbor.predecessor = current_node.node
              neighbor.key.distance = alternative_distance
              queue.decrease_key(neighbor.key)
