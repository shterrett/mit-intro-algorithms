...
3 5 <-- leftover contributes to value
...
7 - inner loop runs once for each stock value if enough cash
    decrements cash until < 0, and then decrements stock. So the
    total loop time is bounded by stock + cash. Because the loop
    isn't nested, it's not stock * cash.
%%% PROBLEM 2(m) ANSWER END %%%
}}

\newcommand{\answerIIn}{\solution{
%%% PROBLEM 2(n) ANSWER START %%%
5 -- Did substitution, but it ooks like I forgot to add the `end` term
%%% PROBLEM 2(n) ANSWER END %%%
}}

\newcommand{\answerIIo}{\solution{
%%% PROBLEM 2(o) ANSWER START %%%
2 -- Did substitution, but it ooks like I forgot to add the `end` term
...
If there is a limit to the number of shares of each stock that can be
purcheased, the problem collapses exactly to the knapsack problem, where each
available share of stock for each company is a separate item, with a cost
(weight) of the initial price and a value of the difference between the final
and initial prices. Because this is a discrete, finite set, nothing needs to be
modified from the knapsack solution to support "buying multiple shares"; the set
of available shares is simply expanded on inititalization so that there is a
single entry for each available share.

stocks = { stock_name => { initial_price:
                           final_price:
                           limit:
                         }
         }
def initialize_available_stock_array(stocks):
  items = []
  cost = []
  value = []
  for each name, data in stocks:
    do data[:limit] times:
      items.append(name)
      cost.append(data[:initial_price]
      value.append(data[:final_price] - data[:initial_price]
  return items, cost, value

This has the same performance characteristics as the knapsack problem, namely
that it runs in O(NC) time where N is the sum of limits of each stock, and C is
the available cash (ie the size of the knapsack). The solution is
psuedopolynomial, then, and for large N relative to C - many more shares of
actual stock than cash with which to purchase them - will run in approximately
polynomial time.

The solutions give the complexity as O(count * total^2) where count is the
number of stocks (not shares), and total is the amount of cash on hand.
This is equivalent to the O(NC) I posited above, where N is the number of
shares summed over the number of stocks, and C is the cash on hand.
To demonstrate equivalence consider the case in which no limits are passed in,
and so each stock is limited only by the amount of cash on hand. Then,
N = SUM(total / initial) for each stock in count. Therefore N is O(total*count)
and C is O(total). O(NC) = O(count * total^2)
\end{codebox}
